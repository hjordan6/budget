...
class _NutritionFormState extends State<NutritionForm> {
  ...
  void initState() {
    ...
    if (widget.initialFiber != null)
      _fiberController.text = widget.initialFiber!.toString();
  }
              ...
              Future<void> submitAdjustment() async {
                ...
                try {
                  ...
                  if (widget.originalQuery != null &&
                      widget.originalQuery!.isNotEmpty) {
                    fullPrompt.writeln(
                      'The user originally described their meal as: "${widget.originalQuery!}"',
                    );
                    fullPrompt.writeln();
                  }
                  ...
                  fullPrompt.writeln();
                  fullPrompt.writeln(
                    'The user has reviewed your analysis and requests the following adjustment:',
                  );
                  fullPrompt.writeln(userRequest);
                  fullPrompt.writeln();
                  fullPrompt.writeln(
                    'Please provide a revised JSON response that incorporates the user\'s feedback. In the "summary" field, clearly explain what changed from the previous analysis and why you made these adjustments.',
                  );
      ...
      Navigator.pushReplacement(
        ...
        MaterialPageRoute(
          builder: (_) => NutritionForm(
            ...
            initialCalories: (result?['nutrients_numeric']?['calories'] as num?)
                ?.toDouble(),
            initialCarbs:
                (result?['nutrients_numeric']?['total_carbs_g'] as num?)
                    ?.toDouble(),
            initialFats: (result?['nutrients_numeric']?['fat_g'] as num?)
                ?.toDouble(),
            initialProtein: (result?['nutrients_numeric']?['protein_g'] as num?)
                ?.toDouble(),
            initialFiber: (result?['nutrients_numeric']?['fiber_g'] as num?)
                ?.toDouble(),
            initialScore: result?['overall_score'] as String?,
            initialBreakdown: result?['summary'] as String?,
            initialVolumePoints: (result?['volume_points'] as num?)?.toDouble(),
            initialNetCarbs:
            ...
            initialAddedSugar:
                (result?['nutrients_numeric']?['added_sugar_g'] as num?)
                    ?.toDouble(),
            initialSodium: (result?['nutrients_numeric']?['sodium_mg'] as num?)
                ?.toDouble(),
            initialFiberLight:
